        ...
        >
          <div class="card-header">
            ...
            <div class="employee-basic">
              ...
              <el-tag 
                :type="employee.status === 'active' ? 'success' : (employee.status === 'inactive' ? 'info' : 'danger')" 
                size="small"
              >
                {{ employee.status === 'active' ? '재직' : (employee.status === 'inactive' ? '휴직' : '잠김') }}
              </el-tag>
            </div>
            <div class="card-actions">
              <div v-if="employee.status !== 'locked'">
              ...
              <div v-else>
                <el-button type="text" @click.stop="unlockEmployee(employee)">
      ...
      <div v-else class="employee-table">
        <el-table :data="filteredEmployees" style="width: 100%">
          ...
          <el-table-column prop="status" label="상태" width="80">
            <template #default="scope">
              <el-tag 
                :type="scope.row.status === 'active' ? 'success' : (scope.row.status === 'inactive' ? 'info' : 'danger')" 
                size="small"
              >
                {{ scope.row.status === 'active' ? '재직' : (scope.row.status === 'inactive' ? '휴직' : '잠김') }}
              </el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="액션" width="120">
            <template #default="scope">
              <div v-if="scope.row.status !== 'locked'">
              ...
              <div v-else>
                <el-button type="text" size="small" @click="unlockEmployee(scope.row)">
                  <el-icon><Unlock /></el-icon>
                  <span>잠금 해제</span>
                </el-button>
              </div>
...
export default {
  ...
  methods: {
    ...
    editEmployee(emp) {
      this.$router.push(`/employee/edit/${emp.id}`);
    },
    deleteEmployee() {
    ...
    unlockEmployee(employee) {
      this.$confirm(`'${employee.name}' 님의 계정 잠금을 해제하시겠습니까?`, '계정 잠금 해제', {
        ...
        if (emp) {
          emp.status = 'active';
          emp.locked = false;
          this.success(`${employee.name} 님의 계정이 활성화되었습니다.`);
        }
      }).catch(() => {
        this.info('잠금 해제가 취소되었습니다.');
      });
    },
    exportEmployees() {
