  ...
  <div class="rule-block">
    ...
    <div v-if="rule.type === 'MANUAL'">
      ...
      <el-form-item label="기본 자동 차감 휴게 시간 (분)" required>
        ...
        />
        <span class="form-description">
          사용자가 휴게를 직접 기록하지 않았을 때 자동으로 차감할 기본 휴게 시간
        </span>
      </el-form-item>
...
export default {
  ...
  watch: {
    'rule.type'(newType) {
      ...
      } else if (newType === 'AUTO' || newType === 'MANUAL') {
    ...
    'workTimeRule.fixedWorkMinutes': {
      handler(newValue) {
        if (newValue && (this.rule.type === 'AUTO' || this.rule.type === 'MANUAL')) {
